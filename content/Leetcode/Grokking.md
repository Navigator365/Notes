...
3. Start of LinkedList Cycle (medium) [Leetcode](https://leetcode.com/problems/linked-list-cycle-ii/) - Practice 1/25
...
6. Problem Challenge 1: Palindrome LinkedList (medium) [Leetcode](https://leetcode.com/problems/palindrome-linked-list/) - REMEMBER: Reversing is O(1)
7. Problem Challenge 2: Rearrange a LinkedList (medium) [Leetcode](https://leetcode.com/problems/reorder-list/)
    - . It sometimes pays to have fast start just after slow
    - When modifying ll links, preserve the next ones
8. Problem Challenge 3: Cycle in a Circular Array (hard) [Leetcode](https://leetcode.com/problems/circular-array-loop/)
    - **Practice 2/1**. Key Ideas: Use a set to track what paths we've already been down, so we can skip those if we come across them

### 3. Pattern: Sliding Window
Key Idea: Using the values at each of pointers, not just their positions, to avoid doing calculations in the middle
1. Introduction
2. Maximum Sum Subarray of Size K (easy)
3. Smallest Subarray with a given sum (easy) [Educative.io](https://leetcode.com/problems/maximum-sum-of-distinct-subarrays-with-length-k/)
	- This is really more of a hashmaps problem than anything else. We're taking  advantage of the fact we can record not just a number's presence, but its location as a value.
	- Also, we start our sliding window as small as possible, and then expand as we get more values
	- **Practice 1/31**
...
6. Longest Substring with Same Letters after Replacement (hard) [LeetCode](https://leetcode.com/problems/longest-repeating-character-replacement/)
	- **Practice 2/3**
	- Key idea: just try to enumerate all cases, I got extremely close to this one without having to look at the answers
